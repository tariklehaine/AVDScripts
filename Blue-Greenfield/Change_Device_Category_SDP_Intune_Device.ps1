...
if ($toAssign.Count -eq 0) {
    Write-Output "✅ No SDP-named Windows devices outside the excluded categories were found. Nothing to do."
    exit
}
...
Write-Output "`n🎉 Finished: assigned '$categoryName' to $($toAssign.Count) SDP* device(s)."
